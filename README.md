...
### 3.1 تحديث نسخة التطبيق الأصلية (APK)

تحديثات OTA أعلاه تغطي فقط ملفات `www/` (JS/HTML/CSS) — أي تعديل في `android/`، `ios/`، `src/native.js`، أو الإضافات الأصلية (Capacitor plugins) يتطلب تثبيت نسخة APK جديدة يدويًا، لأن هذه التعديلات لا يمكن أن تصل عبر OTA.

للمساعدة على اكتشاف هذه الحالة، ينشر `.github/workflows/build-android.yml` بعد كل بناء ناجح على `main`:
- يزيد `versionCode`/`versionName` في `android/app/build.gradle` تلقائيًا قبل البناء.
- يكتب `updates/native-manifest.json` يحتوي رقم النسخة الجديد ورابط تنزيل الـ APK.

التطبيق يقارن (عبر `www/js/native-updater.js`) رقم النسخة الأصلية المثبتة فعليًا على الجهاز (`@capacitor/app` → `getInfo().build`) مع `native-manifest.json`، وإن وجد أحدث يعرض **موديل** (`www/js/app.js` → `promptAvailableUpdates`) بزر "تنزيل وتثبيت" يفتح رابط الـ APK في متصفح النظام لتثبيته يدويًا (نفس آلية التثبيت الجانبي Sideloading الموضحة في القسم 5.1). نفس الموديل يُستخدم أيضًا لعرض زر "تثبيت الآن" الفوري لتحديثات الواجهة (OTA) الجاهزة، بدل انتظار إغلاق التطبيق. يظهر هذا الموديل تلقائيًا عند فتح التطبيق (حسب نفس إعداد الفحص الدوري)، أو فورًا عند الضغط على **فحص التحديثات الآن** بصفحة الإعدادات.
